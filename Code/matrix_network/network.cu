...
Network::Network(int num_l, int* node_list, layer_type* lt, int num_batches) {
    ...
    for(int i = 0; i < num_layers; i++) {
        ...
        outputs_temp = new matrix(num_batches, node_list[num_l - 1]);
        correct_outputs = new matrix(num_batches, node_list[num_l - 1]);
        outputs_temp->move_to_device();
        correct_outputs->move_to_device();
...
void Network::move_layers_device() {
    //layers[1]->move_to_device();

    for(int i = 0; i < num_layers; i++) {
        printf("moving layer %d to device\n", i);
        layers[i]->move_to_device();
    }
}


void Network::set_target_out(float** outs) {
    float* inps = new float[batch_size * num_outputs];
    for(int i = 0; i < batch_size; i++) {
        for(int j = 0; j < num_inputs; j++) {
            inps[i*num_outputs + j] = outs[i][j];
        }
    }
    correct_outputs->set_memory(inps);
}
void Network::set_input(float** ins) {
    float* inps = new float[batch_size * num_inputs];
    for(int i = 0; i < batch_size; i++) {
        for(int j = 0; j < num_inputs; j++) {
            inps[i*num_inputs + j] = ins[i][j];
        }
    }
    layers[0]->outputs->set_memory(inps);
}
...
void Network::back_prop() {
    matrix* nts = correct_outputs;

    for(int i = num_layers - 1; i > 0; i--) {
        layers[i]->back_prop(nts, batch_size);
        nts = NULL;
    }
}
